...
library CartesianMerkleTree {
     ...
     */
    function setHasher(UintCMT storage treaple, function(bytes32, bytes32, bytes32) view returns (bytes32) hash3_)
        internal
    {
     ...
     */
    function getProof(UintCMT storage treaple, uint256 key_, uint32 desiredProofSize_)
        internal
        view
     ...
     * @return The node.
     */
    function getNode(UintCMT storage treaple, uint256 nodeId_) internal view returns (Node memory) {
     ...
     * @return The node.
     */
    function getNodeByKey(UintCMT storage treaple, uint256 key_) internal view returns (Node memory) {
     ...
     * @param hash3_ The hash function that accepts three arguments.
     */
    function setHasher(Bytes32CMT storage treaple, function(bytes32, bytes32, bytes32) view returns (bytes32) hash3_)
        internal
    {
        _setHasher(treaple._treaple, hash3_);
    }
     ...
     * @return CMT proof struct.
     */
    function getProof(Bytes32CMT storage treaple, bytes32 key_, uint32 desiredProofSize_)
        internal
        view
        returns (Proof memory)
    {
     ...
     * @return The node.
     */
    function getNode(Bytes32CMT storage treaple, uint256 nodeId_) internal view returns (Node memory) {
        return _node(treaple._treaple, nodeId_);
    }
     ...
     * @return The node.
     */
    function getNodeByKey(Bytes32CMT storage treaple, bytes32 key_) internal view returns (Node memory) {
     ...
     * @param hash3_ The hash function that accepts three arguments.
     */
    function setHasher(AddressCMT storage treaple, function(bytes32, bytes32, bytes32) view returns (bytes32) hash3_)
        internal
    {
        _setHasher(treaple._treaple, hash3_);
    }
     ...
     * @return CMT proof struct.
     */
    function getProof(AddressCMT storage treaple, address key_, uint32 desiredProofSize_)
        internal
        view
        returns (Proof memory)
    {
     ...
     * @return The node.
     */
    function getNode(AddressCMT storage treaple, uint256 nodeId_) internal view returns (Node memory) {
        return _node(treaple._treaple, nodeId_);
    }
     ...
     * @return The node.
     */
    function getNodeByKey(AddressCMT storage treaple, address key_) internal view returns (Node memory) {
    ...
    function _setDesiredProofSize(CMT storage treaple, uint32 desiredProofSize_) private {
        require(desiredProofSize_ > 0, "CartesianMerkleTree: desired proof size must be greater than zero");

        treaple.desiredProofSize = desiredProofSize_;
    }

    function _setHasher(CMT storage treaple, function(bytes32, bytes32, bytes32) view returns (bytes32) hash3_)
        private
    {
    ...
    function _add(CMT storage treaple, uint256 rootNodeId_, bytes32 key_) private returns (uint256) {
    ...
    function _remove(CMT storage treaple, uint256 rootNodeId_, bytes32 key_) private returns (uint256) {
        Node storage rootNode = treaple.nodes[uint64(rootNodeId_)];
        ...
        if (rootNode.key == key_) {
            ...
            if (leftRootChildNode.key == 0 || rightRootChildNode.key == 0) {
                ...
                rootNodeId_ = leftRootChildNode.key == 0 ? rootNode.childRight : rootNode.childLeft;
    ...
    function _proof(CMT storage treaple, bytes32 key_, uint256 desiredProofSize_) private view returns (Proof memory) {
        desiredProofSize_ = desiredProofSize_ == 0 ? _desiredProofSize(treaple) : desiredProofSize_;
        ...
        while (true) {
            ...
            if (node.key == key_) {
                _addProofSibling(proof_, currentSiblingsIndex_++, treaple.nodes[node.childLeft].merkleHash);
                _addProofSibling(proof_, currentSiblingsIndex_++, treaple.nodes[node.childRight].merkleHash);
            ...
            _addProofSibling(proof_, currentSiblingsIndex_++, treaple.nodes[otherNodeId_].merkleHash);
        }
    ...
    function _addProofSibling(Proof memory proof_, uint256 currentSiblingsIndex_, bytes32 siblingToAdd_) private pure {
        require(currentSiblingsIndex_ < proof_.siblings.length, "CartesianMerkleTree: desired proof size is too low");
    ...
    function _getNodesHash(CMT storage treaple, bytes32 nodeKey_, bytes32 leftNodeKey_, bytes32 rightNodeKey_)
        private
        view
        returns (bytes32)
    {
        function(bytes32, bytes32, bytes32) view returns (bytes32) hash3_ =
            treaple.isCustomHasherSet ? treaple.hash3 : _hash3;

        return hash3_(nodeKey_, leftNodeKey_, rightNodeKey_);
    }

    function _nodeByKey(CMT storage treaple, bytes32 key_) private view returns (Node memory result_) {
        uint256 nextNodeId_ = treaple.merkleRootId;
